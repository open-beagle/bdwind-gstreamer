...
// OnDemandConfig 按需推流配置
type OnDemandConfig struct {
	Enabled              bool          `yaml:"enabled" json:"enabled"`                               // 是否启用按需启动（默认：true）
	IdleTimeout          time.Duration `yaml:"idle_timeout" json:"idle_timeout"`                     // 无客户端后多久停止推流（默认：5秒）
	QuickReconnectWindow time.Duration `yaml:"quick_reconnect_window" json:"quick_reconnect_window"` // 快速重连窗口（默认：10秒）
}
...
func DefaultGStreamerConfig() *GStreamerConfig {
	return &GStreamerConfig{
		...
		OnDemand: DefaultOnDemandConfig(),
	}
}
...
func (c *GStreamerConfig) SetDefaults() {
	...
	c.OnDemand = DefaultOnDemandConfig()
}
...
// DefaultOnDemandConfig 返回默认的按需推流配置
func DefaultOnDemandConfig() OnDemandConfig {
	return OnDemandConfig{
		Enabled:              true,
		IdleTimeout:          5 * time.Second,
		QuickReconnectWindow: 10 * time.Second,
	}
}
